...
public class ExecuteScript extends AbstractCommand {
    ...
    public ArrayList<String> execute(UserInterface userInterface, StorageService ss, String[] args) throws IOException {
        ...
        try {
            ...
            while (fileInterface.hashNextLine()){
                ...
                if (listScriptPath[0].equals("execute_script")) {
                    if (!path.contains(pathToScript.toString())) {
                        ...
                        System.out.println(ANSI_RED + "Вы пытаетесь вызвать скрипт, который вы уже вызывали ранее! Пытаетесь устроить рекурсию?" + ANSI_RESET);
                    }
                } else {
                    CommandsManager.getInstance().executeCommand(fileInterface, ss, line);
                }
            ...
            path.removeAll(path);
            userInterface.writeln("Скрипт выполнен успешно. Его выполнение заняло " + (System.currentTimeMillis() - startTime) + " ms");
        } catch (FileNotFoundException e) {
            userInterface.writeln("Файла не существует.");
        } catch (Exception e) {
