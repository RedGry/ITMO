...
public class CommandsManager {
    ...
    public void executeCommand(UserInterface userInterface, StorageService storageService, String s, DatagramChannel datagramChannel, SocketAddress socketAddress, DataBaseManager dataBaseManager) throws IOException{
        commandsList.add(s);
        String[] parsedCommand = parseCommand(s);
        AbstractCommand command = getCommand(parsedCommand[0]);
        String[] args = Arrays.copyOfRange(parsedCommand, 1, parsedCommand.length);

        command.execute(userInterface, storageService, args, dataBaseManager);
    }

    public void executeCommand(UserInterface userInterface, StorageService storageService, AbstractCommand command, DatagramChannel datagramChannel, SocketAddress socketAddress, DataBaseManager dataBaseManager) throws IOException, InvalidCountOfArgumentException {
        if(!command.getCommand().equals("show")) {
            commandsList.add(command.getCommand());
        }
    ...
    public static void cmdList(){
        ...
        CommandsManager.getInstance().printToClient("suc_history" + "_" + sb.toString());
    }
    ...
    public void sendCollectionToClient(LinkedHashMap<Long, Person> linkedHashMap){
        try{
            CommandsManager.getInstance().getServerDatagramChannel().send(ByteBuffer.wrap("CollAllrt".getBytes()), CommandsManager.getInstance().getSocketAddress());

            try(ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
                ObjectOutputStream objectOutputStream = new ObjectOutputStream(byteArrayOutputStream)){
                objectOutputStream.writeInt(linkedHashMap.size());
                linkedHashMap.forEach((key, person) -> {
                    try {
