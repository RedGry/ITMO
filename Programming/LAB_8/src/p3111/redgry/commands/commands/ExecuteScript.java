                ...
                while (fileInterface.hashNextLine()) {
                    ...
                    if (listScriptPath[0].equals("execute_script")) {
                        if (!path.contains(pathToScript.toString())) {
                            ...
                            CommandsManager.getInstance().executeCommand(fileInterface, ss, line, CommandsManager.getInstance().getServerDatagramChannel(), CommandsManager.getInstance().getSocketAddress(), dataBaseManager);
                        } else {
                            StringBuilder sb = new StringBuilder();
                            sb.append("Рекурсия: \n");
                            sb.append(ANSI_RED + "Вы пытаетесь вызвать скрипт, который вы уже вызывали ранее! Пытаетесь устроить рекурсию?" + ANSI_RESET);
